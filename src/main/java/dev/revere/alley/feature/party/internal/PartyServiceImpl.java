...
public class PartyServiceImpl implements PartyService {
    ...
    public void startFFAMatch(Kit kit, Arena arena, Party party) {
        ...
        Arena selectedArena = this.arenaService.selectArenaWithPotentialTemporaryCopy(arena);
        if (kit.isSettingEnabled(KitSettingHideAndSeek.class)) {
            GameParticipant<MatchGamePlayer> participantA = new TeamGameParticipant<>(toGamePlayer(allPartyPlayers.get(0)));
            GameParticipant<MatchGamePlayer> participantB = new TeamGameParticipant<>(toGamePlayer(allPartyPlayers.get(1)));
            for (int i = 2; i < allPartyPlayers.size(); i++) {
                participantB.addPlayer(toGamePlayer(allPartyPlayers.get(i)));
            }

            this.matchService.createAndStartMatch(kit, selectedArena, participantA, participantB, true, false, false);
            return;
        }
        ...
        this.matchService.createAndStartMatch(
                kit, selectedArena, participants
        );
    }

    private MatchGamePlayer toGamePlayer(Player player) {
        return new MatchGamePlayer(player.getUniqueId(), player.getName());
    }
    ...
    public void createParty(Player player) {
        ...
        // Defensive cleanup: remove any stale party reference or orphaned party
        // 防御性清理：删除任何残留的派对引用或孤立的派对
        if (profile.getParty() != null) {
            profile.setParty(null);
        }
        Party existingParty = this.getPartyByLeader(player);
        if (existingParty != null) {
            this.parties.remove(existingParty);
        }
        existingParty = this.getPartyByMember(player.getUniqueId());
        if (existingParty != null) {
            this.parties.remove(existingParty);
        }
        ...
        SoundUtil.playCustomSound(player, Sound.ENTITY_FIREWORK_ROCKET_TWINKLE, 1.0F, 1.0F);
        ...
        try {
            ...
            for (UUID memberId : new ArrayList<>(party.getMembers())) {
                try {
                    ...
                    Logger.error("Failed to cleanup party member " + memberId + " during disband: " + e.getMessage());
                }
            }
        } finally {
    ...
    public void leaveParty(Player player) {
        ...
        party.notifyParty(this.localeService.getString(GlobalMessagesLocaleImpl.PARTY_PLAYER_LEFT)
                ...
                .replace("{max-size}", "30") //TODO: Implement party size limit with permissions ect... //TODO: 通过权限等机制实现队伍人数限制
        );
