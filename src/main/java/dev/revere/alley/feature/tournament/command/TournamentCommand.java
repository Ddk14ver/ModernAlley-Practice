...
public class TournamentCommand extends BaseCommand {
    ...
    @CommandData(name = "tournament.disband", aliases = {"tournament.cancel"}, permission = "alley.tournament.admin.cancel", description = "Disbands/cancels a tournament.")
    public void disband(CommandArgs command) {
        Player player = command.getPlayer();
        String[] args = command.getArgs();
        ...
        if (active.isEmpty()) {
            player.sendMessage(CC.translate("&cThere are no active tournaments to disband."));
            return;
        }

        Tournament tournamentToDisband = findTournamentFromArgs(player, args, active);
        if (tournamentToDisband == null) return;

        String reason = args.length > 1 ? String.join(" ", Arrays.copyOfRange(args, 1, args.length)) : "Tournament disbanded by an administrator.";
        player.sendMessage(CC.translate("&cDisbanding the " + tournamentToDisband.getDisplayName() + " tournament (#" + tournamentToDisband.getNumericId() + ")..."));

        Bukkit.broadcastMessage(CC.translate("&c&lTournament Disbanded &7(&f#" + tournamentToDisband.getNumericId() + "&7)"));
        Bukkit.broadcastMessage(CC.translate(" &6│ &fHost: &6" + tournamentToDisband.getHostName()));
        Bukkit.broadcastMessage(CC.translate(" &6│ &fReason: &c" + reason));
        Bukkit.broadcastMessage("");

        tournamentService.cancelTournament(tournamentToDisband, reason);
    }
     ...
     * - /tournament join <name>（不含关键字的举办者名称）
     *
     * @param sender     The player executing the command.
     *                   执行命令的玩家。
     ...
     *         连接后的名称，如"name1 and name2"或"name1, name2 and name3"。
     */
