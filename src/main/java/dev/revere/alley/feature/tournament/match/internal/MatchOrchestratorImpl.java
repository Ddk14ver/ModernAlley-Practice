...
public class MatchOrchestratorImpl implements MatchOrchestrator {
    ...
    public MatchProcessingResult processMatchResult(Match match, Tournament tournament) {
        ...
        // Coin rewards
        dev.revere.alley.feature.coin.CoinRewardService coinReward =
                dev.revere.alley.AlleyPlugin.getInstance().getService(dev.revere.alley.feature.coin.CoinRewardService.class);
        winner.getOnlinePlayers().forEach(coinReward::rewardTournamentWin);
        loser.getOnlinePlayers().forEach(coinReward::rewardTournamentLoss);
     ...
     * 从已完成的比赛中解析锦标赛参与者，通过委托给具体比赛的获胜者/失败者参与者，
     * 并经领队UUID进行映射，与原始系统行为保持一致。
     *
     ...
     *                   为true则解析获胜者，为false则解析失败者。
     * @return The resolved TournamentParticipant, or null if not found.
     *         解析出的 TournamentParticipant，如果未找到则返回 null。
     */
     ...
     * the waiting pool.
     * 查找包含特定玩家UUID的锦标赛参与者组，先检查当前参与者和本轮参与者，
     * 然后回退到等待池中查找。
     *
     * @param tournament The tournament context.
     *                    锦标赛上下文。
     ...
     *         包含该UUID的 TournamentParticipant，如果未找到则返回 null。
     */
