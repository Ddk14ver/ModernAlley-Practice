package dev.revere.alley.feature.visibility.internal;
...
public class VisibilityServiceImpl implements VisibilityService {
    ...
    public void handleVisibility(Player viewer, Player target) {
        ...
        switch (viewerProfileState) {
            case LOBBY:
            case WAITING:
                this.handleLobbyAndQueueState(viewer, target, viewerProfile, targetProfileState);
                break;
            case FFA:
     ...
     */
    private void handleLobbyAndQueueState(Player viewer, Player target, Profile viewerProfile, ProfileState targetProfileState) {
        // If the viewer has "hide players" enabled, hide all other players in the lobby
        if (viewerProfile.getProfileData().getSettingData().isHidePlayersEnabled()) {
