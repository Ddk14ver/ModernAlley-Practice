...
public class QueuesMenuModern extends Menu {
    ...
    public Map<Integer, Button> getButtons(Player player) {
        ...
        buttons.put(1, new QueuesButtonModern("&6&lUnranked", Material.IRON_SWORD, 0, Arrays.asList(
                CC.MENU_BAR,
                "&7Casual 1v1s with",
                "&7no loss penalty.",
                "",
        ...
        buttons.put(3, new QueuesButtonModern("&6&lUnranked Duos", Material.GOLDEN_SWORD, 0, Arrays.asList(
                CC.MENU_BAR,
                "&7Casual 2v2s with",
                "&7no penalty loss",
                "",
        ...
        buttons.put(5, new QueuesButtonModern("&6&lRanked", Material.DIAMOND_SWORD, 0, Arrays.asList(
                CC.MENU_BAR,
                "&7Competitive 1v1s with",
                "&7Elo rating, gains/losses.",
                "",
                "&6│ &fPlayers: &6" + queueService.getPlayerCountOfGameType("Ranked"),
                "",
                this.getLore(profile, QueueType.RANKED),
                CC.MENU_BAR
        )));

        buttons.put(7, new QueuesButtonModern("&6&lFFA", Material.GOLDEN_AXE, 0, Arrays.asList(
                CC.MENU_BAR,
                "&7Free for all with",
                "&7infinity respawns.",
                "",
        ...
        switch (profile.getQueueType()) {
            ...
            case BOTS:
                ...
                for (Kit kit : AlleyPlugin.getInstance().getService(KitService.class).getKits()) {
                    if (!botService.isKitSupported(kit)) continue;
                    slot = this.skipIfSlotCrossingBorder(slot);
