...
/**
 * Match pearl listener for handling ender pearl mechanics during matches.
 * 比赛末影珍珠监听器，用于处理比赛期间的末影珍珠机制。
 */
public class MatchPearlListener implements Listener {

    //TODO: Locale (im not touching pearls, thats on you mr insurant)
    // 待办：本地化（我不碰珍珠，那是你的事，insurant 先生）
    ...
    public void onPearlLaunch(ProjectileLaunchEvent event) {
        ...
        int pearlCooldownSeconds = getPearlCooldownSeconds(profile);
        if (pearlCooldownSeconds > 0 && hasPearlCooldown(player, event)) {
            return;
        }

        if (!profile.getMatch().getKit().isSettingEnabled(KitSettingRaiding.class)) {
            if (pearlCooldownSeconds > 0) {
                applyCooldown(player, pearlCooldownSeconds);
            }
            return;
        }
    ...
    public void onPlayerTeleport(PlayerTeleportEvent event) {
        ...
        int pearlCooldownSeconds = getPearlCooldownSeconds(player);
        if (pearlCooldownSeconds > 0) {
            applyCooldown(player, pearlCooldownSeconds);
        }
    }
    ...
    private int getPearlCooldownSeconds(Profile profile) {
        if (profile == null
                || profile.getState() != ProfileState.PLAYING
                || profile.getMatch() == null
                || profile.getMatch().getState() != MatchState.RUNNING) {
            return 0;
        }

        KitSettingPearlCooldownImpl setting = profile.getMatch().getKit()
                .getSetting(KitSettingPearlCooldownImpl.class);
        return setting == null ? 0 : Math.max(0, setting.getValue());
    }

    private int getPearlCooldownSeconds(Player player) {
        return getPearlCooldownSeconds(AlleyPlugin.getInstance()
    ...
    private boolean isFenceGateRestricted(Block block, String direction) {
        ...
        if (block.getRelative(BlockFace.UP).getType() == Material.OAK_FENCE_GATE ||
    ...
    private boolean isChestRestricted(Block block, String direction) {
        ...
        if (!block.getRelative(BlockFace.UP).getType().isSolid() ||
                block.getRelative(BlockFace.UP).getType() == Material.OAK_FENCE_GATE) {
            return false;
        }
    ...
    private boolean isForbiddenTeleportBlock(Block block) {
        ...
        return type == Material.IRON_BARS ||
    ...
    private boolean isFenceGateTeleportRestricted(Block block, Player player, PlayerTeleportEvent event) {
        ...
        // 情况1：上方和当前方块都是栅栏门，上方处于关闭状态
        if (upper.getType() == Material.OAK_FENCE_GATE &&
                block.getType() == Material.OAK_FENCE_GATE &&
                !((Gate) upper.getBlockData()).isOpen() &&
                !((Gate) block.getBlockData()).isOpen() &&
        ...
        // 情况2：当前方块是栅栏门（关闭），下方是栅栏门（关闭），上方不是栅栏门
        if (upper.getType() != Material.OAK_FENCE_GATE &&
                block.getType() == Material.OAK_FENCE_GATE &&
                !((Gate) block.getBlockData()).isOpen() &&
                lower.getType() == Material.OAK_FENCE_GATE &&
                !((Gate) lower.getBlockData()).isOpen()) {
            sendInvalidPearlFeedback(player, event);
            return true;
        }
    ...
    private void setPearlMetadata(EnderPearl pearl, Player player, Block block, Block blockAbove) {
        ...
        if (shouldSetTaliMetadata(block, blockAbove, direction)) {
            pearl.setMetadata("tali", new FixedMetadataValue(AlleyPlugin.getInstance(), true));
        }

        // Set "cpearl" metadata for various block types
        // 为各种方块类型设置 "cpearl" 元数据
        if (shouldSetCpearlMetadata(block, direction)) {
            pearl.setMetadata("cpearl", new FixedMetadataValue(AlleyPlugin.getInstance(), true));
        }
    ...
    private boolean shouldSetTaliMetadata(Block block, Block blockAbove, String direction) {
        ...
        return block.getRelative(BlockFace.UP).getType() != Material.OAK_FENCE_GATE &&
                block.getRelative(BlockFace.UP).getType().isSolid() &&
    ...
    private boolean isSlab(Block block) {
        Material type = block.getType();
        return type.name().endsWith("_SLAB");
    }

    private boolean isStainedGlassPane(Material type) {
    ...
    private boolean isStair(Block block) {
        ...
        return type == Material.ACACIA_STAIRS ||
                ...
                type == Material.STONE_BRICK_STAIRS ||
    ...
    private void cancelPearlAndRefund(Player player, ProjectileLaunchEvent event, String message) {
        event.setCancelled(true);
        // Only refund if the pearl was actually consumed; event cancel may not restore it
        if (getConfig().getBoolean("return-pearl", true)) {
    ...
    private void applyCooldown(Player player, int seconds) {
        CooldownService cooldownService = AlleyPlugin.getInstance().getService(CooldownService.class);
        long durationMillis = Math.max(0, seconds) * 1000L;
        Cooldown cooldown = cooldownService.getCooldown(player.getUniqueId(), CooldownType.ENDER_PEARL);
        if (cooldown != null && cooldown.getDurationMillis() != durationMillis) {
        ...
        if (cooldown == null) {
            cooldown = new Cooldown(CooldownType.ENDER_PEARL, durationMillis,
            ...
            cooldownService.addCooldown(player.getUniqueId(), CooldownType.ENDER_PEARL, cooldown);
        }
    ...
    private ConfigurationSection getConfig() {
        return AlleyPlugin.getInstance().getService(ConfigService.class).getPearlConfig();
    }
}
