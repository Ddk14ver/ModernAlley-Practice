...
public class GomokuMatch extends DefaultMatch implements GomokuPlayable {
    private final GomokuGame game;

    public GomokuMatch(Queue queue, Kit kit, Arena arena, boolean ranked,
    ...
    public void handleRoundStart() {
        ...
        this.game.startTwoSides(getParticipantA(), getParticipantB());
    }
    ...
    private void finishGame(GameParticipant<MatchGamePlayer> winningParticipant) {
        GameParticipant<MatchGamePlayer> losingParticipant =
                winningParticipant == getParticipantA() ? getParticipantB() : getParticipantA();
        losingParticipant.getPlayers().forEach(gamePlayer -> {
            gamePlayer.setDead(true);
            gamePlayer.setEliminated(true);
        });

        Player winner = firstOnlinePlayer(winningParticipant);
        Player loser = firstOnlinePlayer(losingParticipant);
