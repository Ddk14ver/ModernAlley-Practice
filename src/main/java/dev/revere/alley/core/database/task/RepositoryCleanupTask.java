 ...
 * @since 09/03/2025
 *
 * 仓库清理任务，定期清理过期的战斗记录、决斗请求、快照和组队请求。
 * Repository cleanup task, periodically cleaning up expired combat records, duel requests, snapshots and party requests.
 */
    ...
    public void run() {
        ...
        if (!combatService.getCombatMap().isEmpty()) {
            combatService.getCombatMap().forEach((uuid, combat) -> {
                ...
                if (player != null && combatService.isExpired(player)) {
