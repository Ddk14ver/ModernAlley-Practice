...
public class RankedUnbanCommand extends BaseCommand {
    ...
    public void onCommand(CommandArgs command) {
        ...
        // Save ban info before clearing
        String reason = profile.getProfileData().getRankedBanReason();
        if (reason == null || reason.isEmpty()) reason = "N/A";
        String banId = profile.getProfileData().getRankedBanId();
        if (banId == null || banId.isEmpty()) banId = "N/A";

        // Clear all ban fields
        profile.getProfileData().setRankedBanned(false);
        profile.getProfileData().setRankedBanExpiry(0L);
        profile.getProfileData().setRankedBanReason("");
        profile.getProfileData().setRankedBanId("");
