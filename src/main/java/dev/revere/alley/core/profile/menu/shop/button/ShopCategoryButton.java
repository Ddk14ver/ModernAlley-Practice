...
public class ShopCategoryButton extends Button {
    ...
    public ItemStack getButtonItem(Player player) {
        ...
        if (cosmeticType == CosmeticType.TITLE) {
            ...
            ProfileData profileData = AlleyPlugin.getInstance().getService(ProfileService.class)
                    .getProfile(player.getUniqueId()).getProfileData();
            totalCount = (int) titleService.getSortedTitles().stream().filter(t -> t.isPurchasable()).count();
            ownedCount = profileData.getUnlockedTitles().size();
        } else {
